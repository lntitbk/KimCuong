		...
		Crafty.c("2D", {
			...
			_defineGetterSetter_defineProperty: function () {
				Object.defineProperty(this, 'x', {
					set: function (v) {this._attr('_x', v);}, get: function () {return this._x;}, configurable: true
				});

				Object.defineProperty(this, 'y', {
					set: function (v) {this._attr('_y', v);}, get: function () {return this._y;}, configurable: true
				});

				Object.defineProperty(this, 'w', {
					set: function (v) {this._attr('_w', v);}, get: function () {return this._w;}, configurable: true
				});

				Object.defineProperty(this, 'h', {
					set: function (v) {this._attr('_h', v);}, get: function () {return this._h;}, configurable: true
				});

				Object.defineProperty(this, 'z', {
					set: function (v) {this._attr('_z', v);}, get: function () {return this._z;}, configurable: true
				});
...
//My code
					...
					var off = getPos(obj);
					if (Crafty.support.prefix !== "webkit" && Crafty.support.prefix !== "Moz")
						off = {x: x, y: y};
//			iterateObj(off);
					return off;
				},
...
//			alert(x + ";" + Crafty.stage.x + ";" + document.body.scrollLeft + ";" + document.documentElement.scrollLeft + ";" + Crafty.viewport._x);
					return {
				...
				scale: (function () {
					...
					return function (amt) {
						var bounds = Crafty.map.boundaries(),
							...
							innerStyle = Crafty.stage.inner.style;

//                this._zoom = final_zoom;
						this._zoom = czoom = final_zoom = amt ? amt : 1;
						act.width = bounds.max.x - bounds.min.x;
						act.height = bounds.max.y - bounds.min.y;
						var new_s = {
						...
						Crafty.viewport.pan('reset');
						innerStyle['transform'] = innerStyle[prop] = 'scale(' + this._zoom + ',' + this._zoom + ')';

						final_zoom = 1 - final_zoom;
						innerStyle.top = Math.floor(-this.height * final_zoom / 2) + "px";
//				innerStyle.left = Math.floor(-this.width * final_zoom/2) + "px";
						var offset = Crafty.DOM.inner(Crafty.stage.elem);
//				alert(offset.x);
						Crafty.stage.x = offset.x + Math.floor(this.width * final_zoom / 2);
						Crafty.stage.y = offset.y;

						if (Crafty.canvas._canvas) {
							Crafty.canvas.context.scale(czoom, czoom);
//							Crafty.canvas._canvas.style.top = Math.floor(-this.height * final_zoom / 2) + "px";
							Crafty.canvas._canvas.style.left = Math.floor(this.width * final_zoom / 2) + "px";
							Crafty.DrawManager.drawAll();
						}
...
//            Crafty.addEvent(this, window, "resize", Crafty.viewport.reload);
					...
					var elem = Crafty.stage.elem.style, inStyle = Crafty.stage.inner.style,
						offset;
					...
					inStyle.width = this.width + "px";
					inStyle.height = this.height + "px";
					...
					if (Crafty.mobile) {
						...
						setTimeout(resizeViewportHdl, 100);
...
//				alert(offset.x);
						...
						Crafty.stage.y = offset.y;
						window.addEventListener("resize", function() {setTimeout(Crafty.viewport.reload,500);}, false);
					}
		...
		Crafty.extend({
			...
			mouseDispatch: function (e) {
				...
				var maxz = -1,
					...
					type = e.type;
//alert(e.clientX + ";" + e.clientY + " and " + pos.x + ';' + pos.y);
...
var lastHeight = -1, resetCount = 0;
function resetViewport() {
	if ((Crafty.mobile === 'android' && Crafty.support.prefix === 'webkit' && Crafty.support.version === 533)) {
		$("meta[name=viewport]").remove();
		var meta = document.createElement('meta'), content = 'width=' + Crafty.viewport.width;
		meta.setAttribute('name', 'viewport');
...
function resizeViewport (w, h) {
	if (Crafty.mobile) {
		var _w = $(window).width(), //window.innerWidth || (window.document.documentElement.clientWidth || window.document.body.clientWidth),
			_h = $(window).height();//*window.innerHeight || (window.document.documentElement.clientHeight || window.document.body.clientHeight);
		if ((Crafty.mobile === 'ipod' || Crafty.mobile === 'iphone') && navigator.userAgent.indexOf('CriOS') < 0) {
			var realH = 60 * _w / (window.orientation === 0 ? screen.width : screen.height) + _h;
			if (realH < h) _h = Math.floor(realH);
		}
		if (Math.abs(lastHeight - _h) < 50) {
			return false;
		}
		lastHeight = _h;
		resetViewport();
		if (w / h > _w / _h) {
			...
			Crafty.viewport.scale(Math.floor(_h / h * 1000) / 1000);
		}
	}
	return true;
}
function resizeViewportHdl() {
	resetCount++;
	if (resetCount < 10 && !resizeViewport(Crafty.viewport.width, Crafty.viewport.height)) {
		setTimeout(resizeViewportHdl, 100);
	} else {
		setTimeout(function () {window.scrollTo(0, 0);}, 0)
	};
